...
class Monitoramento(ZabbixAPI):
    ...
    def get_itservice_pid(self, grupo):
        parentId = self.zapi.service.get({"selectParent": "extend", "selectTrigger": "extend", "expandExpression": "true","filter": {"name": grupo}})[0]['serviceid']
        return parentId
    ...
    def create_child_itservices(self, host, grupo):
        self.zapi.service.create({"name": host, "algorithm": "1", "showsla": "1", "goodsla": "99.99", "sortorder": "1",
                                  "parentid": self.get_itservice_pid(grupo)})

    def get_itservice_pid_child(self, host):
        parentIdChild = self.zapi.service.get({"selectParent": "extend", "selectTrigger": "extend", "expandExpression": "true","filter": {"name": host}})[0]['serviceid']
    ...
    def create_child_itservices_trigger(self, host, item):
        self.zapi.service.create(
            {"name": self.get_item_triggerName(host, item), "algorithm": "1", "showsla": "1", "goodsla": "99.99",
             "sortorder": "1", "parentid": self.get_itservice_pid_child(host),
             "triggerid": self.get_item_triggerid(host, item)})

    def get_itservices(self,parent_id=0):
        itServices = self.zapi.service.get({"selectParent": parent_id, "selectTrigger": "extend"})
        listaServicos = []
        for x in itServices:
            listaServicos += [x]
    ...
    def get_sla(self,service_id_list,tm_from,tm_to):
    ...
    def get_service(self,group_id):
        from_p = "1/10/2019 00:10:00"
        to_futuro = "9/10/2019 19:35:00"
        ...
        nomeGrupo = self.get_hostgroups_name(group_id)
        hosts = self.get_hosts(nomeGrupo)
        host_service = []
        ok_tm = 0
        problem_tm = 0
        down_tm = 0
        ...
        for hostName in hosts:
            service_id_item = self.get_itservice_pid_child(hostName)
            service_list = self.get_itservices(service_id_item)
            sla = self.get_sla(service_id_item, passado_timestamp, futuro_timestamp)
            ok_tm = sla[service_id_item]["sla"][0]["okTime"]
            a["sla"] = ok_tm
            problem_tm = sla[service_id_item]["sla"][0]["problemTime"]
            down_tm = sla[service_id_item]["sla"][0]["downtimeTime"]
            sla_percent = sla[service_id_item]["sla"][0]["sla"]



            host_service +=  service_list
        print(a)

        # for i in host_service:
        #     print("ServiceID: {} Nome: {} GoodSLA: {:.2f}".format(i["serviceid"],i["name"],float(i["goodsla"])))
        #     print("OkTime: {} Tempo de Problema: {} Down Time: {} SLA: {:.2f}\n".format(ok_tm, problem_tm, down_tm,
        #                                                                                 sla_percent))
    ...
    def teste(self,temp):
        from_timestamp = time.strftime("%d/%m/%Y %H:%M:%S", time.localtime(temp))
...
a = Monitoramento("http://192.168.3.200/zabbix", "Admin", "zabbix")


# host_serviceid = a.get_itservice_pid_child("desk01")
#
# service = a.pega_services(host_serviceid)
# service[0]["hostname"] = "teste"
#
# print(service[0]["hostname"])

#a.get_service(24)

# print(a.get_itservices())