...
class Monitoramento(ZabbixAPI):
    ...
    def get_itservices(self,hostid=''):
        itServices = self.zapi.service.get({"selectParent": "extend", "selectTrigger": "extend","parentids": 1051})
        listaServicos = []
        for x in itServices:
    ...
    def get_service(self, group_id):
        ...
        for hostName in hosts:
            ...
            service_list = self.get_itservices()
            goodsla = service_list[0]["goodsla"]
            sla = self.get_sla(service_id_item, passado_timestamp, futuro_timestamp)

            print(service_list)

            # print(type(sla[service_id_item]["status"]))
            status = "Diponivel" if int(sla[service_id_item]["status"]) == 0 else "Indisponivel"
            ...
            print("Nome: {: >35} "
                  "\nStatus: {: >33} "
                  "\nSLA Aceitavel: {: >24.2f} %  "
                  "\nDisponibilidade: {: >22.2f} %"
                  "\nTempo Disponivel: {: >23} "
                  "\nTempo de Falha: {: >23}"
                  "\nTempo de Manuntenção: {: >13}\n".format(hostName,
                  status,float(goodsla),float(sla_percent),ok_tm,down_tm,problem_tm))
...
# print(a.get_sla(1050,1569899400,1570935540))
# a.get_service([4])

#print(a.get_itservices(10084))
# print((a.get_itservices(16651)))
...
#print(a.print_triggers_hosts(10084))